/*
 * SF2 live interaction trace v8 — log-only instrumentation (no game-state mutation).
 * Extends live_boot_trace.js with:
 *  - s3ePointer* hooks: prove whether the logic loop READS input (touch counter,
 *    X/Y read on return) — the dojo-interaction mission hinges on this.
...
function attachAll() {
    ...
    hookS3e('s3ePointerRegister', function (args) {
        // s3ePointerRegister(callback, eventType, userData) — if the game
