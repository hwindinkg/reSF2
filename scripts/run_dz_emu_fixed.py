#!/usr/bin/env python3
"""Fixed ARM Unicorn emulation of s3eCompressionDecomp for forge.xml type-4.
Uses correct DZ field layout and absolute offsets."""
...
# Phase 1: all PLT -> ret0
for addr in plt_by_name.values():
    w32(addr, 0xe51ff004)
    w32(addr + 4, ret0)
...
for name, fid in hook_map.items():
    if name not in plt_by_name:
        print(f"  NOT IN PLT: {name}")
        continue
    tramp = make_trampoline(fid)
    addr = plt_by_name[name]
    w32(addr, 0xe51ff004)
    w32(addr + 4, tramp)
    print(f"  {name} -> fid={fid} @ PLT 0x{addr:05x}")
...
pos += nfiles * 6  # folder_idx(2) + file_number(2) + flags(2)
pos += 3  # lengths trailer

# File table: f0=offset, f1=comp_size, f2=uncomp_size, f3=type
...
w32(OUT_PTR_ADDR, 0)  # output pointer (null = auto-alloc)
w32(OUT_SZ_ADDR, 0x100000)  # output size hint

# The global vars that s3eCompressionDecompInit reads
# From Ghidra: DAT_000d8514 (VA 0xd8514) = input ptr
#              DAT_000d8518 (VA 0xd8518) = input size
# Actual VA in the ELF = Ghidra VA - 0x10000
GLBL_IN_PTR = 0xc8514
GLBL_IN_SZ = 0xc8518
print(f"\nSetting globals: DAT_000d8514@0x{GLBL_IN_PTR:x}=0x{IN_BUF:x}")
print(f"                 DAT_000d8518@0x{GLBL_IN_SZ:x}={len(comp_data)}")
w32(GLBL_IN_PTR, IN_BUF)
w32(GLBL_IN_SZ, len(comp_data))

FUNC_ADDR = 0x51c1c  # s3eCompressionDecomp (Ghidra 0x61c1c)
RET_ADDR  = 0x9FFFF00

SP = STACK + 0x80000
mu.reg_write(UC_ARM_REG_SP, SP)
mu.reg_write(UC_ARM_REG_LR, RET_ADDR)
mu.reg_write(UC_ARM_REG_R0, len(comp_data))  # param_1 = input size
mu.reg_write(UC_ARM_REG_R1, IN_BUF)           # param_2 = input ptr
mu.reg_write(UC_ARM_REG_R2, OUT_PTR_ADDR)     # param_3 = &output_ptr
mu.reg_write(UC_ARM_REG_R3, OUT_SZ_ADDR)      # param_4 = &output_size
w32(SP, 4)  # param_5 (type hint) = 4 = type-4

# ── Trace hooks ──
def trace_code(mu, address, size, user_data):
    if address == 0x51414:
        r0 = mu.reg_read(UC_ARM_REG_R0)
        r1 = mu.reg_read(UC_ARM_REG_R1)
        r2 = mu.reg_read(UC_ARM_REG_R2)
        print(f"  > s3eCompressionDecompInit(type={r0}, readcb=0x{r1:x}, flags={r2})")
    elif address == 0x51a10:
        print(f"  > s3eCompressionDecompRead enter")
    elif address == 0x51f60:  # FUN_00061f60
        print(f"  > FUN_00061f60 (type-4 read internal)")
    elif address == 0x520bc:  # FUN_000620bc
        r0 = mu.reg_read(UC_ARM_REG_R0)
        print(f"  > FUN_000620bc(ctx=0x{r0:x})")
  elif address == 0x51668:  # before memset call in init
      r0 = mu.reg_read(UC_ARM_REG_R0)
      r1 = mu.reg_read(UC_ARM_REG_R1)
      ...
      thumb = (cpsr >> 5) & 1
      print(f"  > Init before memset: R0=0x{r0:x} R1=0x{r1:x} R12=0x{r12:x} LR=0x{lr:x} Thumb={thumb}")
      # Check PLT entry
      data = bytes(mu.mem_read(0xc444, 8))
      print(f"  > PLT memset @0xc444: {data.hex()}")

mu.hook_add(UC_HOOK_CODE, trace_code)

print(f"\n=== Emulating s3eCompressionDecomp @ 0x{FUNC_ADDR:05x} ===")
print(f"  R0={len(comp_data)}  R1=0x{IN_BUF:08x}  R2=0x{OUT_PTR_ADDR:08x}  R3=0x{OUT_SZ_ADDR:08x}")
print(f"  [SP]=type=4")
...
try:
    ...
    r0 = mu.reg_read(UC_ARM_REG_R0)
    r12 = mu.reg_read(UC_ARM_REG_R12)
    print(f"\n!!! CRASH PC=0x{pc:05x} LR=0x{lr:05x} SP=0x{sp:08x} R12=0x{r12:08x}")
